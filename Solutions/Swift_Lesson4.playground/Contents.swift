...
let addTenClosure = { (value: Int) -> Int in
    return value + 10
}
...
sayHello()   // must use parentheses because parameters are ()
...
doNothing()   // still requires parentheses
...
counter1()   // 1
counter1()   // 2
counter1()   // 3  — remembers its own state

let counter2 = makeCounter()
counter2()   // 1 — separate captured scope
...
let results = numbers.filter({ (number: Int) -> Bool in
    return number > 4
})
...
let sortedB = numbers.sorted(by: { a, b in
    return a < b
})
...
let sortedC = numbers.sorted(by: { a, b in
    a < b
})
...
let sortedD = numbers.sorted { a, b in
    a < b
}
...
let longNamesB = names.filter {
    $0.count > 3
}

print(longNamesB)   // ["Alice", "Charlie", "Derek"]
...
let passing = scores.filter({ (score: Int) -> Bool in
    return score >= 70
})
print("Passing scores: \(passing)")

// Step 3: Add 5 bonus points to each passing score
let bonusPoints = passing.map({ (score: Int) -> Int in
    return score + 5
})
print("Bonus points: \(bonusPoints)")


let passingShort = scores.filter { $0 >= 70 }
let bonusPointsShort = passingShort.map { $0 + 5 }

print("Passing (shorthand): \(passingShort)")
print("Bonus points (shorthand): \(bonusPointsShort)")
...
let nums = firstElement(of: [1, 2, 3])        // T = Int
let words = firstElement(of: ["a", "b", "c"]) // T = String
...
// -----------------------------
func smallest<T: Comparable>(in array: [T]) -> T? {
    return array.min()
}

let s1 = smallest(in: [3, 1, 9])          // OK — Int is Comparable
let s2 = smallest(in: ["b", "a", "c"])    // OK — String is Comparable
...
struct Kennel<T: Animal> {
    var pet: T
}

let d = Kennel(pet: Dog())   // OK — Dog inherits Animal
...
struct Record<T: BaseModel & Identifiable & Printable> {
    var value: T
}
...
func allMatch<T: Equatable>(array: [T]) -> Bool {
    for val in array {
        if val != array[0] {
            return false
        }
