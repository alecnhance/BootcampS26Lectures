...
struct MiniChallenge1: View {
    @State var list: [String] = ["Strawberry", "Oatmeal", "Ginger", "Beef", "Bread"]
    var body: some View {
        VStack {
            Text("Grocery List").font(.title).bold()
            ForEach(list, id: \.self) { item in
                Text(item).foregroundStyle(.red).padding(3)
            }
            Button("Click here to remove the last item") {
                list.popLast()
            }
        }
    }
...
struct SubviewExample: View {
    ...
    var body: some View {
        VStack(spacing: 12) {
            top
            ProfileRow(name: "Alec", role: "iOS Developer")
            ProfileRow(name: "Sam", role: "Designer")
            ProfileRow(name: "Jamie", role: "Product Manager")
        }
        .padding()
    }
    var top: some View {
...
struct BindingExampleView: View {
    ...
    var body: some View {
        VStack {
            ...
            CounterButtonSubview(value: $number)
        }
        .padding()
    }
}
...
// 3. Create a reusable subview with + and – buttons that
//    takes a @Binding Int.
...
struct ThreeNumberChallengeView: View {
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            Text("Number two: \(b)")
            Text("Number three: \(c)")
            NumberAdjuster(value: $a)
            NumberAdjuster(value: $b)
            NumberAdjuster(value: $c)
...
@available(iOS 17.0, *)
struct ObservableEnvExample: View {
    @Environment(AppModel.self) var model    // reading from environment
...
// ----------------------------------------------------------
// MINI-CHALLENGE — Using @Observable (No MVVM, No Navigation)
// ----------------------------------------------------------
//
// Goal:
...
struct MoodDisplayView: View {
    ...
    var body: some View {
        Text("Current Mood: \(model.currentMood)")
            .font(.largeTitle)
            .padding()
    }
}
...
struct MoodButtonsView: View {
    ...
    var body: some View {
        HStack(spacing: 20) {
            Button("Happy") { model.currentMood = "Happy" }
            Button("Neutral") { model.currentMood = "Neutral" }
            Button("Sad") { model.currentMood = "Sad" }
        }
        .font(.title)
        .padding()
    }
...
struct ObservableMiniChallenge: View {
    ...
    var body: some View {
        ...
        .environment(model)   // insert observable model
    }
}
